...
void CTplotData(const char * mainFile = "", const char * bgFile = "", Double_t
		a = 0, Double_t b = 0, Double_t c = 0, Double_t d = 0, Double_t e = 0, Int_t f, Int_t g){
	...
	Double_t BackgroundTimeRatio = 1.0/1.0; //2000.0/500.0;
	...
	TF1 *fit1 = new TF1("fit1",Gauss, a,b,7); // range & number of parameters 
	TF1 *fit2 = new TF1("fit2",Gauss, a,b,7); // range & number of parameters 
	TF1 *fit3 = new TF1("fit3",Gauss, a,b,7); // range & number of parameters 
	fit1->SetParameters(500., c, 300.0, 100.0, 0.0, 0.0, 0.0); 
	fit1->SetLineWidth(1);
	fit1->SetLineColor(kGreen);	
	fit2->SetParameters(500., c, 300.0, 100.0, 0.0, 0.0, 0.0); 
	fit2->SetLineWidth(1);
	fit2->SetLineColor(kGreen);	
	fit3->SetParameters(500., c, 300.0, 100.0, 0.0, 0.0, 0.0); 
	fit3->SetLineWidth(1);
	...
	Int_t LowerWindowBin = f;
	Int_t UpperWindowBin = g;

	Double_t MeasuredSignal = hist1->Integral(LowerWindowBin,UpperWindowBin);

	Double_t windowLowerBound = hist1->GetBinLowEdge(LowerWindowBin);
	Double_t windowUpperBound = hist1->GetBinLowEdge(UpperWindowBin)
	...
	MeasuredSignal = hist2->Integral(LowerWindowBin,UpperWindowBin);
	arrow->DrawArrow(windowLowerBound, 0.0, windowLowerBound, (8000),0.005,"<");
	arrow->DrawArrow(windowUpperBound, 0.0, windowUpperBound, (8000),0.005,"<");
	arrow->Draw("same");
